...
$(".asset").dblclick(function(){

    if( !$(this).hasClass('used') ){
        //check if asset has already been used
        //if it doesn’t have the class, proceed

        if(downloadUrls.length<5){
            // check the lenght of the downloadURLs array to
            // determine whether or not to add to it

            $(".assetimage",this).clone().appendTo(".assetblock");
             let addUrl = $(".assetimage",this).data('url'); // use data attribute that gets data-url value, instead of href
             downloadUrls.push(addUrl);
             setupDownload();

             // denote that the asset has already been used
             $(this).addClass('used');

        }else{
            // if you’ve reached the limit, you can’t add any more
            // you can maybe show a message that explains 
            // the 5-item limit
        }
    }
...
$(".refresh").click(function(){         
    ...
    // reset their used status
    $('.asset').removeClass('used'); 
    downloadUrls = [];
});
